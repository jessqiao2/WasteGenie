...
public class CouncilTrackingAdapter extends RecyclerView.Adapter<CouncilTrackingAdapter.MyViewHolder> implements Filterable {
    ...
    public CouncilTrackingAdapter(Context context, ArrayList<BinData> list){
    ...
    @NonNull
    @Override
    public CouncilTrackingAdapter.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType){
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.list_route_tracking, parent, false);
        return new CouncilTrackingAdapter.MyViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull CouncilTrackingAdapter.MyViewHolder holder, int position){
    ...
    public Filter getFilter() {
        return councilFilter;
    }

    private final Filter councilFilter = new Filter() {

        @Override
        protected FilterResults performFiltering(CharSequence councilFilter) {

            ArrayList<BinData> filteredCouncilList = new ArrayList<>();

            // if the user doesn't choose a selection, show the full list of the tracking status
            // for all trucks
            if (councilFilter == null || councilFilter.length() == 0) {
                filteredCouncilList.addAll(fullList);
            } else {
                // loop through all the data within the list to find those which
                // contain the filtered truck id
                for (BinData bindata : fullList) {
                    if (bindata.getCouncilName().contains(councilFilter))
                        filteredCouncilList.add(bindata);
                }
            }

            FilterResults results = new FilterResults();
            results.values = filteredCouncilList;
            results.count = filteredCouncilList.size();
            return results;
        }
